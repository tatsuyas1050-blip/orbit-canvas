...
EventBridge が週1で Lambda を起動し、次を実行します（「ソノッキーの部屋」の継続支援が主目的）。

1. **開発日誌のAI下書き** — 直近のGitコミットを GitHub API で取得し、Claude が日誌の下書きを生成
2. **ネタ提案** — 直近の天文現象（計算）を種に、星空コラム/ギャラリー/ヨゾラジ のネタを提案
3. **LINEリマインド** — コンテンツ鮮度を集計し「今週の一手＋ネタ案」を自分にプッシュ通知

> ℹ️ **星空情報局（天文現象カレンダー）は手動運用**（管理画面のAI一括インポート）。
> このジョブは天文現象を**自動でカレンダーに投入しません**。計算した今後の現象は「ネタ提案」のタイムリーな種としてのみ使います。

生成物（開発日誌）は **下書き（draft）** で止まります。公開は管理画面の「週次コックピット → 下書き受信箱」で〔公開〕を押したときだけ（承認制）。
